...
It excludes (v1+): user-defined generics, sized numeric variants (`real32`, `integer32`, etc.), rank-3+ arrays / a shape language, structs with methods, sum types / pattern matching, operator definitions, modules with submodule interface/implementation split, FFI, parallel-for, and a package manager.
