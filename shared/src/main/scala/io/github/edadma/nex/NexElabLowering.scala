...
protected trait NexElabLowering extends NexElabState:
  ...
  protected def lowerExpr(e: TExpr): TExpr = e match
    ...
    case TSlice(a, lo, hi, inc, st, p, t)  => TSlice(lowerExpr(a), lo.map(lowerExpr), hi.map(lowerExpr), inc, st.map(lowerExpr), p, t)
    case TSlice2(a, rAx, cAx, p, t)    =>
      def lowAxis(s: TAxisSpec): TAxisSpec = s match
        ...
        case TAxisRange(lo, hi, i, st) => TAxisRange(lo.map(lowerExpr), hi.map(lowerExpr), i, st.map(lowerExpr))
    ...
    case TInterpStringLit(parts, p, t) =>
      ...
      val lowered = parts.map {
        case TInterpExpr(x, sp) => TInterpExpr(lowerExpr(x), sp)
        case other          => other
      }
  ...
  protected def rootSym(e: TExpr): Option[Symbol] = e match
    ...
    case TSlice(r, _, _, _, _, _, _) => rootSym(r)
  ...
  protected def walkChildren(e: TExpr, reads: Set[Int], names: Map[Int, String]): Unit =
    e match
      ...
      case TInterpStringLit(parts, _, _) =>
        parts.foreach {
          case TInterpExpr(x, _) => walkForMutations(x, reads, names)
          case _              => ()
        }
