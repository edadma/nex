...
class NexLLVMCodegen:
  ...
  /** Identity-keyed map from each TLambda node to its synthesized
    ...
    */
  private val lambdaTable = new java.util.IdentityHashMap[TLambda, LambdaInfo]
  ...
  /** While emitting a synthetic lambda function body, this maps each
    ...
    * [[locals]]).
    */
  private var lambdaCaptures: Map[Int, (Int, Type, CaptureMode)] = Map.empty
  private var lambdaEnvTy:     String                              = ""

  /** Set of all Symbol ids that bind a `var` (i.e., need by-ref capture
    * when referenced inside a lambda).
    */
  private val varBindings = mutable.Set.empty[Int]
  ...
  private def collectVarBindings(tp: TProgram): Unit =
  ...
  private def collectLambdas(tp: TProgram): Unit =
  ...
  private def emitLambdaFunctions(): Unit =
  ...
  /** Construct a closure value `{ fn_ptr, env_ptr }` from a TLambda. The
    ...
    * the closure literal.
    */
  private def emitLambdaConstruct(lam: TLambda): String =
  ...
  /** Emit a TyFunc-typed call: extract `{fn_ptr, env_ptr}` and call
    ...
    */
  private def emitClosureCall(callee: TExpr, args: List[TExpr], retT: Type): String =
