...
class NexLLVMCodegen
  ...
  //
  // Representation (see [[NexLLVMState.llvmType]]): `{ i32 tag, [N x i64] }`
  // where N = max field-count across the enum's variants. Each variant
  // field occupies one i64 payload slot — scalars are bit-cast / zext'd
  // into the slot at construction and inverted at pattern dispatch:
  //   integer →  i64 (identity)
  //   real    →  bitcast to/from i64
  //   bool    →  zext i1 → i64  /  trunc i64 → i1
  //   string  →  ptrtoint / inttoptr (descriptor ptr ↔ i64)
  // Aggregate fields (complex / tuple / struct / array / nested enum)
  // are rejected up front by [[enumFieldSlotType]].
  // ===========================================================================
  ...
  /** Build an enum value for variant `idx` of `te` with `args` filling
    ...
    */
  private def emitVariantConstruct(te: TyEnum, idx: Int, args: List[TExpr]): String =
  ...
  /** Emit a `match` expression as a `switch` over the runtime tag with
    ...
    */
  private def emitMatch(scrutinee: TExpr, cases: List[TMatchCase], resultT: Type): String =
  ...
  private def bindPattern(pat: TPattern, scrutV: String, te: TyEnum): Unit =
    pat match
      case TWildcardPat(_) => ()
      case TVarPat(sym, _) =>
        // Bind whole scrutinee to a fresh alloca. Inc here so the
        // binding owns an independent share; the caller's dec then
